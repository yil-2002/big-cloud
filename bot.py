...
#playbackRateBtn{
  ...
  color:#fff;border-radius:6px;padding:3px 8px;font-size:12px;cursor:pointer;flex-shrink:0;
}
/* ── Nav tugmalar (playlist: keyingi/oldingi) ── */
.nav-btn{
  position:absolute;top:50%;transform:translateY(-50%);
  width:44px;height:44px;border-radius:50%;
  background:rgba(0,0,0,.35);color:#fff;border:none;
  display:flex;align-items:center;justify-content:center;
  font-size:20px;z-index:15;cursor:pointer;transition:opacity .2s;
}
.nav-btn.hidden{display:none;}
.nav-btn:disabled{opacity:.25;pointer-events:none;}
#videoPrevBtn,#imagePrevBtn{left:10px;}
#videoNextBtn,#imageNextBtn{right:10px;}
/* ── Seek flash (double-tap ±10s) ── */
.seek-flash{
  position:absolute;top:0;bottom:0;width:40%;
  display:flex;align-items:center;justify-content:center;
  font-size:15px;color:#fff;background:rgba(255,255,255,0);
  pointer-events:none;opacity:0;transition:opacity .15s;z-index:8;
}
.seek-flash.left{left:0;justify-content:flex-start;padding-left:10%;}
.seek-flash.right{right:0;justify-content:flex-end;padding-right:10%;}
.seek-flash.show{opacity:1;background:rgba(255,255,255,.06);}
.seek-flash span{
  background:rgba(0,0,0,.5);border-radius:50%;width:56px;height:56px;
...
#imageHeader{
  display:flex;align-items:center;gap:10px;padding:14px 14px 8px;flex-shrink:0;z-index:16;
}
#imageTitle{flex:1;color:#fff;font-size:15px;font-weight:600;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;}
#imageBody{
  flex:1;display:flex;align-items:center;justify-content:center;padding:8px 12px 20px;
...
#imageEl.no-transition{transition:none;}
#imageCounter{
  position:absolute;bottom:10px;left:0;right:0;text-align:center;
  color:rgba(255,255,255,.6);font-size:12px;z-index:16;pointer-events:none;
}
</style>
</head>
<body>
...
<div id="videoOverlay">
  <div id="videoHeader">
    ...
    <button class="v-btn" id="playbackRateBtn">1x</button>
    <a class="v-btn" id="videoDownload" href="#" download>⬇</a>
  </div>
  <video id="mainVideo" playsinline preload="metadata"></video>
  <div class="seek-flash left" id="seekFlashLeft"><span>⏪10</span></div>
  <div class="seek-flash right" id="seekFlashRight"><span>10⏩</span></div>
  <button class="nav-btn" id="videoPrevBtn">‹</button>
  <button class="nav-btn" id="videoNextBtn">›</button>
...
<div id="imageOverlay">
  ...
  <div id="imageBody">
    ...
    <button class="nav-btn" id="imagePrevBtn">‹</button>
    <button class="nav-btn" id="imageNextBtn">›</button>
...
const state = {
  ...
  currentFiles: []   // hozirgi ro'yxatdagi fayllar — playlist/album navigatsiya uchun
};
...
function renderFiles(files, append){
  ...
  if (!append) { container.innerHTML = ''; state.currentFiles = []; }
  state.currentFiles = state.currentFiles.concat(files);
...
const rateBtn      = document.getElementById('playbackRateBtn');
const seekFlashL   = document.getElementById('seekFlashLeft');
const seekFlashR   = document.getElementById('seekFlashRight');
let controlsTimer;
let videoList = [], videoIndex = -1;
...
function updateVideoNavButtons(){
  ...
  videoNextBtn.disabled = videoIndex >= videoList.length - 1;
  const single = videoList.length <= 1;
  videoPrevBtn.classList.toggle('hidden', single);
  videoNextBtn.classList.toggle('hidden', single);
}

async function openVideo(f, list){
  videoList = list || state.currentFiles.filter(x => x.category === 'video');
  videoIndex = videoList.findIndex(x => x.id === f.id);
  if (videoIndex === -1) { videoList = [f]; videoIndex = 0; }
  videoOverlay.classList.add('show');
  await playCurrentVideo();
}

async function playCurrentVideo(){
  const f = videoList[videoIndex];
  if (!f) return;
  ...
  mainVideo.src = '';
  mainVideo.playbackRate = PLAYBACK_RATES[rateIdx];
  rateBtn.textContent = PLAYBACK_RATES[rateIdx] + 'x';
...
function videoNext(){ if (videoIndex < videoList.length - 1){ videoIndex++; playCurrentVideo(); } }
function videoPrev(){ if (videoIndex > 0){ videoIndex--; playCurrentVideo(); } }
videoPrevBtn.onclick = (e) => { e.stopPropagation(); videoPrev(); };
videoNextBtn.onclick = (e) => { e.stopPropagation(); videoNext(); };

rateBtn.onclick = (e) => {
  e.stopPropagation();
  rateIdx = (rateIdx + 1) % PLAYBACK_RATES.length;
  mainVideo.playbackRate = PLAYBACK_RATES[rateIdx];
  rateBtn.textContent = PLAYBACK_RATES[rateIdx] + 'x';
  showVideoControls();
};
...
mainVideo.addEventListener('ended', () => {
  playPauseBtn.textContent='▶'; playIcon.classList.remove('hidden');
  if (videoIndex < videoList.length - 1) videoNext();   // playlist: keyingisiga o'tish
});
...
// ── Bitta/qo'sh tegish: markazda play/pause, chapda/o'ngda ±10s ──
let vTapTimer = null, vTapCount = 0, vTapX = 0;
function flashSeek(side){
  const el = side === 'left' ? seekFlashL : seekFlashR;
  el.classList.add('show');
  clearTimeout(el._t);
  el._t = setTimeout(() => el.classList.remove('show'), 450);
}
function seekBy(sec, side){
  if (!mainVideo.duration) return;
  mainVideo.currentTime = Math.min(Math.max(0, mainVideo.currentTime + sec), mainVideo.duration);
  flashSeek(side);
  showVideoControls();
}
mainVideo.addEventListener('click', e => {
  const rect = mainVideo.getBoundingClientRect();
  vTapX = e.clientX - rect.left;
  vTapCount++;
  if (vTapCount === 1){
    vTapTimer = setTimeout(() => { togglePlay(); vTapCount = 0; }, 260);
  } else {
    clearTimeout(vTapTimer);
    vTapCount = 0;
    const w = rect.width;
    if (vTapX < w * 0.35) seekBy(-10, 'left');
    else if (vTapX > w * 0.65) seekBy(10, 'right');
    else togglePlay();
  }
});

// ── Swipe: keyingi/oldingi video (playlist) ──
let vSwipeX = 0, vSwipeY = 0, vSwiped = false;
videoOverlay.addEventListener('touchstart', e => {
  showVideoControls();
  if (e.touches.length === 1){
    vSwipeX = e.touches[0].clientX; vSwipeY = e.touches[0].clientY; vSwiped = false;
  }
}, {passive:true});
videoOverlay.addEventListener('touchmove', () => { vSwiped = true; }, {passive:true});
videoOverlay.addEventListener('touchend', e => {
  if (!vSwiped || !e.changedTouches.length) return;
  const dx = e.changedTouches[0].clientX - vSwipeX;
  const dy = e.changedTouches[0].clientY - vSwipeY;
  if (Math.abs(dx) > 70 && Math.abs(dx) > Math.abs(dy) * 1.5){
    if (dx < 0) videoNext(); else videoPrev();
  }
}, {passive:true});
...
// ══════════════════════════════════════════════════════════
// IMAGE VIEWER (album swipe + pinch-zoom + pan)
// ══════════════════════════════════════════════════════════
...
let imgList = [], imgIndex = -1;
let imgScale = 1, imgX = 0, imgY = 0;
let pinchStartDist = 0, pinchStartScale = 1;
let panStartX = 0, panStartY = 0, isPanning = false;
let iSwipeX = 0, iSwipeY = 0, iSwiping = false;
let iTapTimer = null, iTapCount = 0;

function applyImgTransform(withTransition){
  imgEl.classList.toggle('no-transition', !withTransition);
  imgEl.style.transform = `translate(${imgX}px, ${imgY}px) scale(${imgScale})`;
}
function resetImgTransform(withTransition){
  imgScale = 1; imgX = 0; imgY = 0;
  applyImgTransform(withTransition);
}
function clampPan(){
  // Zoom qilinganda rasm ekrandan chiqib ketmasligi uchun cheklov
  const maxOffset = (imgScale - 1) * 140;
  imgX = Math.min(Math.max(imgX, -maxOffset), maxOffset);
  imgY = Math.min(Math.max(imgY, -maxOffset), maxOffset);
}
...
function updateImageNav(){
  const single = imgList.length <= 1;
  imagePrevBtn.classList.toggle('hidden', single);
  imageNextBtn.classList.toggle('hidden', single);
  imagePrevBtn.disabled = imgIndex <= 0;
  imageNextBtn.disabled = imgIndex >= imgList.length - 1;
  imageCounter.textContent = single ? '' : `${imgIndex + 1} / ${imgList.length}`;
}

async function openImage(f, list){
  imgList = list || state.currentFiles.filter(x => x.category === 'photo');
  imgIndex = imgList.findIndex(x => x.id === f.id);
  if (imgIndex === -1) { imgList = [f]; imgIndex = 0; }
  imageOverlay.classList.add('show');
  await loadCurrentImage();
}

async function loadCurrentImage(){
  const f = imgList[imgIndex];
  if (!f) return;
...
function imageNext(){ if (imgIndex < imgList.length - 1){ imgIndex++; loadCurrentImage(); } }
function imagePrev(){ if (imgIndex > 0){ imgIndex--; loadCurrentImage(); } }
imagePrevBtn.onclick = (e) => { e.stopPropagation(); imagePrev(); };
imageNextBtn.onclick = (e) => { e.stopPropagation(); imageNext(); };
...
// ── Pinch-zoom / pan / swipe ──
imageBody.addEventListener('touchstart', e => {
  if (e.touches.length === 2){
    ...
    if (imgScale > 1.02){
      isPanning = true;
      panStartX = e.touches[0].clientX - imgX;
      panStartY = e.touches[0].clientY - imgY;
    } else {
      iSwipeX = e.touches[0].clientX; iSwipeY = e.touches[0].clientY; iSwiping = false;
    }
  }
}, {passive:true});

imageBody.addEventListener('touchmove', e => {
  if (e.touches.length === 2){
    const dist = getDist(e.touches);
    if (pinchStartDist > 0){
      imgScale = Math.min(Math.max(pinchStartScale * (dist / pinchStartDist), 1), 4);
      clampPan();
      applyImgTransform(false);
    }
  } else if (e.touches.length === 1){
    if (isPanning){
      imgX = e.touches[0].clientX - panStartX;
      imgY = e.touches[0].clientY - panStartY;
      clampPan();
      applyImgTransform(false);
    } else {
      iSwiping = true;
...
imageBody.addEventListener('touchend', e => {
  isPanning = false;
  if (imgScale <= 1.02 && iSwiping && e.changedTouches.length){
    const dx = e.changedTouches[0].clientX - iSwipeX;
    const dy = e.changedTouches[0].clientY - iSwipeY;
    if (Math.abs(dx) > 70 && Math.abs(dx) > Math.abs(dy) * 1.5){
      if (dx < 0) imageNext(); else imagePrev();
    }
  }
  if (imgScale < 1) resetImgTransform(true);
  iSwiping = false;
}, {passive:true});
...
imageEl.addEventListener('click', e => {
  iTapCount++;
  if (iTapCount === 1){
    iTapTimer = setTimeout(() => { iTapCount = 0; }, 280);
  } else {
    clearTimeout(iTapTimer);
    iTapCount = 0;
    if (imgScale > 1.02) resetImgTransform(true);
    else { imgScale = 2.5; applyImgTransform(true); }
  }
});
