...
def user_menu_kb(show_admin: bool = False):
    ...
    WEBAPP_BASE_URL = os.getenv("WEBAPP_BASE_URL", f"https://your-bot.onrender.com")
    kb.add(InlineKeyboardButton(
        "☁️ Cloud Drive (Mini App)",
        web_app=types.WebAppInfo(url=f"{WEBAPP_BASE_URL}/webapp")
    ))
    # ────────────────────────────────────────────────────────────────────────
    kb.add(
...
function openVideo(f){
  ...
  dlBtn.href = '/webapp/download/' + f.id;
  dlBtn.download = f.name;
  mainVideo.src = '';
  // Stream video bevosita Telegram file URL orqali
  mainVideo.src = '/webapp/download/' + f.id;
...
function openImage(f){
  ...
  dlBtn.href = '/webapp/download/' + f.id;
  dlBtn.download = f.name;
  document.getElementById('imageEl').src = '/webapp/download/' + f.id;
  imageOverlay.classList.add('show');
}
document.getElementById('imageClose').onclick = () => {
  imageOverlay.classList.remove('show');
  document.getElementById('imageEl').src = '';
};
