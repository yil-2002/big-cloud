...
SORT_OPTIONS = {
    "date_desc": ("sub.date DESC",            "📅 Yangi → Eski"),
    "date_asc":  ("sub.date ASC",             "📅 Eski → Yangi"),
    "name_asc":  ("sub.file_name ASC",        "🔤 A → Z"),
    "name_desc": ("sub.file_name DESC",       "🔤 Z → A"),
    "size_desc": ("sub.size DESC",            "📦 Katta → Kichik"),
    "size_asc":  ("sub.size ASC",             "📦 Kichik → Katta"),
    "pinned":    ("sub.pinned DESC, sub.date DESC", "📌 Avval pinlanganlar"),
}

# Foydalanuvchi sort tanlovi: {account_id: sort_key}  (xotirada, sessiya davomida)
user_sort_pref: dict = {}

def get_user_sort(account_id: int) -> str:
    return user_sort_pref.get(account_id, "date_desc")

def set_user_sort(account_id: int, sort_key: str):
    if sort_key in SORT_OPTIONS:
        user_sort_pref[account_id] = sort_key
...
async def get_files_with_total(
    ...
    sort_by: SORT_OPTIONS kaliti (default: date_desc)
    """
    order_sql, _ = SORT_OPTIONS.get(sort_by, SORT_OPTIONS["date_desc"])
...
def sort_menu_kb(current_sort: str = "date_desc"):
    """Saralash menyusi — 3+3+1 tartibda"""
    kb = InlineKeyboardMarkup(row_width=3)
    keys = list(SORT_OPTIONS.keys())
    # 1-qator: date_desc, date_asc, name_asc
    row1 = []
    for k in keys[:3]:
        _, label = SORT_OPTIONS[k]
        mark = "✅ " if k == current_sort else ""
        row1.append(InlineKeyboardButton(f"{mark}{label}", callback_data=f"sort_apply:{k}"))
    kb.add(*row1)
    # 2-qator: name_desc, size_desc, size_asc
    row2 = []
    for k in keys[3:6]:
        _, label = SORT_OPTIONS[k]
        mark = "✅ " if k == current_sort else ""
        row2.append(InlineKeyboardButton(f"{mark}{label}", callback_data=f"sort_apply:{k}"))
    kb.add(*row2)
    # 3-qator: pinned
    for k in keys[6:]:
        _, label = SORT_OPTIONS[k]
        mark = "✅ " if k == current_sort else ""
        kb.add(InlineKeyboardButton(f"{mark}{label}", callback_data=f"sort_apply:{k}"))
    kb.add(InlineKeyboardButton("🔙 Orqaga / Назад", callback_data="menu"))
    return kb

def sort_indicator_btn(sort_key: str) -> InlineKeyboardButton:
    """Fayllar ro'yxatida saralash ko'rsatgich tugmasi"""
    _, label = SORT_OPTIONS.get(sort_key, SORT_OPTIONS["date_desc"])
    return InlineKeyboardButton(f"🔀 Saralash: {label}", callback_data="sort_menu")
...
async def show_files_page(chat_id, account_id, is_admin, ctx, page=0,
                           extra_where="", extra_params=(), sort_by: str = None):
    if sort_by is None:
        sort_by = get_user_sort(account_id) if account_id else "date_desc"
    offset        = page * PAGE_SIZE
    rows, total   = await get_files_with_total(
        account_id, is_admin, extra_where, extra_params, PAGE_SIZE, offset, sort_by
    )
    ...
    for i, row in enumerate(rows):
        ...
        if i == len(rows) - 1:
            ...
            if nav:
                kb.add(*nav)
            kb.add(sort_indicator_btn(sort_by))
            kb.add(InlineKeyboardButton("🏠 Menyu / Меню", callback_data="menu"))
...
async def cb_category(call: CallbackQuery):
    ...
    sort_by = get_user_sort(account_id)
    try: await call.message.delete()
    except Exception: pass
    await bot.send_message(call.message.chat.id, f"<b>{CAT_NAMES_USER[cat]}</b>", parse_mode="HTML")
    await show_files_page(call.message.chat.id, account_id, False, f"cat:{cat}", page, where, params, sort_by)
    await call.answer()
...
async def cb_admin_category(call: CallbackQuery):
    ...
    if cat not in CAT_MAP:
        await call.answer(); return
    where, params = CAT_MAP[cat]
    account_id_admin = await get_account_id(call.from_user.id)
    sort_by = get_user_sort(account_id_admin or 0)
    try: await call.message.delete()
    except Exception: pass
    await bot.send_message(call.message.chat.id, f"<b>{CAT_NAMES_ADMIN[cat]}</b>", parse_mode="HTML")
    await show_files_page(call.message.chat.id, 0, True, f"admin:cat:{cat}", page, where, params, sort_by)
    await call.answer()
...
async def cb_admin_account_files(call: CallbackQuery):
    ...
    admin_acc_id = await get_account_id(call.from_user.id)
    sort_by = get_user_sort(admin_acc_id or 0)
    try: await call.message.delete()
    except Exception: pass
...
@dp.callback_query_handler(lambda c: c.data == "sort_menu")
async def cb_sort_menu(call: CallbackQuery):
    """Saralash menyusini ko'rsatish"""
    account_id = await get_account_id(call.from_user.id)
    is_admin_flag = is_admin_mode(call.from_user.id)
    if not is_admin_flag and not account_id:
        await call.answer("🔐 Avval kiring!", show_alert=True)
        return
    current = get_user_sort(account_id or 0)
    await safe_edit(
        call,
        "🔀 <b>Saralash tartibi / Порядок сортировки</b>\n\nSaralash turini tanlang:",
        reply_markup=sort_menu_kb(current)
    )
    await call.answer()

@dp.callback_query_handler(lambda c: c.data.startswith("sort_apply:"))
async def cb_sort_apply(call: CallbackQuery):
    """Saralash turini tanlash va menyu orqali qaytish"""
    account_id = await get_account_id(call.from_user.id)
    is_admin_flag = is_admin_mode(call.from_user.id)
    if not is_admin_flag and not account_id:
        await call.answer("🔐 Avval kiring!", show_alert=True)
        return
    sort_key = call.data.split(":", 1)[1]
    if sort_key not in SORT_OPTIONS:
        await call.answer(); return
    set_user_sort(account_id or 0, sort_key)
    _, label = SORT_OPTIONS[sort_key]
    await call.answer(f"✅ {label}", show_alert=False)
    # Menyuga qaytish
    await safe_edit(
        call,
        f"✅ Saralash o'rnatildi: <b>{label}</b>\n\nMenyudan fayl kategoriyasini tanlang:",
        reply_markup=(admin_menu_kb() if is_admin_flag else
                      user_menu_kb(show_admin=is_admin_flag))
    )
...
async def cb_search_by_tag(call: CallbackQuery):
    ...
    tag   = parts[1]
    page  = int(parts[2])
    sort_by = get_user_sort(account_id or 0)
    try: await call.message.delete()
    except Exception: pass
...
def _file_row_to_json(row, tags=None):
    return {
        "id": row["id"],
        "name": row["file_name"],
...
async def api_files_handler(request):
    ...
    files = [_file_row_to_json(r, tags_map.get(r["id"], [])) for r in rows]
...
<title>☁️ Cloud Storage</title>
<script src="https://telegram.org/js/telegram-web-app.js"></script>
<style>
  :root{
    --bg:#0f1115; --bg2:#171a21; --card:#1e222b; --border:#2a2f3a;
    --text:#eef1f6; --muted:#8a92a3; --accent:#4f8cff; --accent2:#3f74d9;
    --danger:#ff5c6c; --success:#33c46d; --radius:14px;
  }
  [data-theme="light"]{
    --bg:#f4f6fa; --bg2:#ffffff; --card:#ffffff; --border:#e3e7ee;
  ...
  *{box-sizing:border-box; -webkit-tap-highlight-color:transparent;}
  ...
  .title{font-size:17px; font-weight:700; display:flex; align-items:center; gap:6px;}
  .icon-btn{
  ...
  .chips{display:flex; gap:6px; overflow-x:auto; margin-top:10px; padding-bottom:2px; scrollbar-width:none;}
  ...
  #ptr{
    text-align:center; font-size:12px; color:var(--muted); height:0; overflow:hidden;
  ...
  .fab{
    ...
    background:var(--accent); color:#fff; border:none; font-size:26px; box-shadow:0 6px 18px rgba(0,0,0,.3);
  ...
  .sheet{
    background:var(--bg2); width:100%; max-width:520px; border-radius:16px 16px 0 0;
    padding:16px; max-height:75vh; overflow-y:auto;
  }
  .sheet h3{margin:0 0 10px 0; font-size:15px;}
  .sheet-row{
...
<div id="offline-banner">📡 Internet aloqasi yo'q / Нет подключения</div>

<div class="topbar">
  <div class="topbar-row">
    <div class="title">☁️ Cloud Storage</div>
    <div style="display:flex; gap:8px;">
      <button class="icon-btn" id="viewToggle" title="Ko'rinish">▦</button>
      <button class="icon-btn" id="themeToggle" title="Tema">🌙</button>
    </div>
  </div>
  <div class="search-row">
    <input class="search-input" id="searchInput" placeholder="🔍 Qidirish / Поиск...">
  </div>
...
<div id="ptr">🔄 Yangilanmoqda...</div>

<div class="content">
  <div id="fileContainer" class="grid"></div>
  <div id="loadingMore" class="loading" style="display:none;">Yuklanmoqda...</div>
</div>

<button class="fab" id="uploadFab">＋</button>
<input type="file" id="fileInput" multiple>
<div id="dropzone">📤 Fayllarni shu yerga tashlang</div>
...
function applyTheme(t){
  ...
  localStorage.setItem('cloud_theme', t);
}
let savedTheme = localStorage.getItem('cloud_theme') ||
...
// ─── View mode (grid / list) ───────────────────────────────────────────────
let viewMode = localStorage.getItem('cloud_view') || 'grid';
const container = document.getElementById('fileContainer');
function applyView(){
  container.className = viewMode;
  document.getElementById('viewToggle').textContent = viewMode === 'grid' ? '▦' : '☰';
}
applyView();
document.getElementById('viewToggle').onclick = () => {
  viewMode = viewMode === 'grid' ? 'list' : 'grid';
  localStorage.setItem('cloud_view', viewMode);
  applyView();
};

// ─── State ──────────────────────────────────────────────────────────────────
let state = { category: 'all', folder: '', search: '', page: 0, loading: false, hasMore: true };
...
async function loadFolders(){
  try{
    const data = await api('/webapp/folders');
    const box = document.getElementById('folderChips');
    box.innerHTML = '<div class="chip active" data-folder="">📁 Barcha papkalar</div>' +
      data.folders.map(f => `<div class="chip" data-folder="${f.name}">${f.label}</div>`).join('');
...
function renderFiles(files, append){
  ...
  if (!append && files.length === 0){
    container.innerHTML = '<div class="empty">😔 Fayl topilmadi<br>Hozircha bu yerda hech narsa yo\'q</div>';
    return;
  }
  ...
  files.forEach(f => {
    ...
    card.className = 'card';
    card.innerHTML = `
      ${f.pinned ? '<div class="f-pin">📌</div>' : ''}
      <div class="f-icon">${f.icon}</div>
      <div class="f-name">${escapeHtml(f.name)}</div>
      <div class="f-meta">${fmtSize(f.size)} • ${f.date}</div>
    `;
    card.onclick = () => openFileSheet(f);
...
async function loadFiles(append = false){
  ...
  try{
    const params = new URLSearchParams({
      category: state.category, folder: state.folder, search: state.search, page: state.page
    });
    const data = await api('/webapp/files?' + params.toString());
    renderFiles(data.files, append);
    state.hasMore = data.has_more;
  }catch(e){
    toast('❌ ' + e.message);
    if (!append) container.innerHTML = '<div class="empty">Xatolik yuz berdi</div>';
  }finally{
    state.loading = false;
...
function openFileSheet(f){
  ...
  content.innerHTML = `
    ...
    <div class="sheet-row"><span>Hajmi / Размер</span><b>${fmtSize(f.size)}</b></div>
    <div class="sheet-row"><span>Sana / Дата</span><b>${f.date}</b></div>
    <div class="sheet-row"><span>Papka / Папка</span><b>${escapeHtml(f.folder)}</b></div>
    ${f.tags && f.tags.length ? `<div class="sheet-row"><span>Teglar</span><b>${f.tags.map(t=>'#'+t).join(' ')}</b></div>` : ''}
    <div style="display:flex; gap:8px; margin-top:14px;">
      <button class="btn" style="flex:1" id="dlBtn">⬇️ Yuklab olish</button>
...
async function uploadFiles(fileList){
  for (const file of fileList){
    ...
    try{
      ...
      const msg = data.files && data.files[0] && data.files[0].duplicate
        ? '✅ ' + file.name + ' saqlandi! (diskda mavjud edi)'
        : '✅ ' + file.name + ' saqlandi!';
      toast(msg);
    }catch(e){
...
async def on_startup(dp):
    ...
    await site.start()
    logger.info(f"✅ Bot ishga tushdi! Port: {port} | Web App: /webapp")
