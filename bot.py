        ...
        await conn.execute("""
            ...
            BEGIN
                ...
                IF EXISTS (
                    ...
                    ALTER TABLE folders ALTER COLUMN date TYPE BIGINT
                        USING CASE
                            WHEN date ~ '^[0-9]+$' THEN date::bigint
                            ELSE EXTRACT(EPOCH FROM date::timestamp)::bigint
                        END;
                END IF;
                ...
                IF EXISTS (
                    ...
                    ALTER TABLE files ALTER COLUMN date TYPE BIGINT
                        USING CASE
                            WHEN date ~ '^[0-9]+$' THEN date::bigint
                            ELSE EXTRACT(EPOCH FROM date::timestamp)::bigint
                        END;
                ...
                IF EXISTS (
                    ...
                    ALTER TABLE accounts ALTER COLUMN created_at TYPE BIGINT
                        USING CASE
                            ...
                            ELSE EXTRACT(EPOCH FROM created_at::timestamp)::bigint
                        END;
                END IF;
                ...
                IF EXISTS (
                    ...
                    ALTER TABLE telegram_bindings ALTER COLUMN bound_at TYPE BIGINT
                        USING CASE
                            WHEN bound_at ~ '^[0-9]+$' THEN bound_at::bigint
                            ELSE EXTRACT(EPOCH FROM bound_at::timestamp)::bigint
                        END;
                END IF;
            END $$;
        """)
...
async def get_files_with_total(
    ...
    async with db_pool.acquire() as conn:
        ...
        if not is_admin:
            where_parts.append(f"account_id = ${param_idx}")
