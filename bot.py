...
async def get_files_with_total(
    ...
    Oldin 2 ta so'rov ketardi, endi 1 ta.
    """
    async with db_pool.acquire() as conn:
        if is_admin:
            ...
            """
            rows = await conn.fetch(q, *extra_params)
        else:
            if extra_where:
                ...
                q = f"""
                    SELECT *, COUNT(*) OVER() AS total_count
                    FROM ({SELECT_SQL}) sub
                    WHERE account_id = $1 AND {shifted}
                    ORDER BY date DESC
                    LIMIT {limit} OFFSET {offset}
                """
                rows = await conn.fetch(q, account_id, *extra_params)
            else:
                q = f"""
                    SELECT *, COUNT(*) OVER() AS total_count
                    FROM ({SELECT_SQL}) sub
                    ...
                    LIMIT {limit} OFFSET {offset}
                """
                rows = await conn.fetch(q, account_id)
...
async def process_search(message: Message, state: FSMContext):
    ...
    for row in rows:
        ...
        if is_admin_flag and row["account_id"] != account_id:
            owner_text = f"\n👤 <b>@{row['owner_name'] or 'noma\'lum'}</b>"
        caption = (
